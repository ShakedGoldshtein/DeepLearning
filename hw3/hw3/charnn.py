...
def char_maps(text: str):
    ...
    # ====== YOUR CODE: ======
    char_to_idx = {char: i for i, char in enumerate(sorted(set(text)))}
...
def chars_to_labelled_samples(text: str, char_to_idx: dict, seq_len: int, device="cpu"):
    ...
    samples = embedded_text[:-1].unfold(0, seq_len, seq_len)  
  
    samples = samples.permute(0, 2, 1)

    label_indices = embedded_text[1:].argmax(dim=1) 
    labels = label_indices.unfold(0, seq_len, seq_len) 
    # ========================

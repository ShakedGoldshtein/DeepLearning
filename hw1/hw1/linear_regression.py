...
def top_correlated_features(df: DataFrame, target_feature, n=5):
    ...
    # ====== YOUR CODE: ======
    corr_series = df.corr()[target_feature].drop(target_feature)
    neg_abs = -corr_series.abs()
    sorted_idx = neg_abs.sort_values().index
    top_features_idx = sorted_idx[:n] #got the correct indices
    
    top_n_features = top_features_idx.tolist()
    top_n_corr = corr_series[top_features_idx].values
    # ========================
...
def r2_score(y: np.ndarray, y_pred: np.ndarray):
    ...
    # ====== YOUR CODE: ======
    y_avg = y.mean()
    r2 = 1 - np.divide(mse_score(y,y_pred),mse_score(y, y_avg))
    # ========================
    return r2
